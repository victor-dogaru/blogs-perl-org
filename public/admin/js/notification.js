  ...
  $('.invitation-row').each(function(idx, invitation) {
    ...
    var modal = invitationModal.clone();
      ...
      $(modal).on('click', function() {
        ...
        $('.modal-footer').html('<button class="btn btn-primary btn-ok modal-accept" data-dismiss="modal" onclick="' + '' + 'location.pathname=' +"'/notification/invitation/" + InvitationData.notifications[idx].generic_id  + '/user/' + InvitationData.username + '/mark-read/true' + "'" +'">' + 'Accept' + '</button>' + '<a type="button" class="btn  btn-danger modal-reject" data-dismiss="modal">Reject</a>');
      });
